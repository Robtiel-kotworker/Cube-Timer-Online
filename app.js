...
function useLeaderboard() {
  ...
  return { entries, loading, error, submitScore, adminRemoveEntry, adminBanUsername, adminResetAll };
}

/* ============================================================================
 * lib: "Wer ist online?" – Presence
 * ----------------------------------------------------------------------------
 * Es gibt kein echtes Session-System (siehe useAuth) – "online" bedeutet hier:
 * der Browser des Nutzers hat innerhalb der letzten PRESENCE_ONLINE_THRESHOLD_MS
 * einen Ping an die presence-Tabelle gesendet. Die Liste im Dialog zeigt zudem
 * alle Nutzer, deren letzter Ping bis zu 24 Stunden zurückliegt.
 * ==========================================================================*/
const PRESENCE_HEARTBEAT_MS = 30000; // alle 30 Sekunden ein "Ping"
const PRESENCE_ONLINE_THRESHOLD_MS = 90000; // Ping jünger als 90s = online (3× Heartbeat, tolerant bei Netz-Hakern)
const PRESENCE_RECENT_WINDOW_MS = 24 * 60 * 60 * 1000; // Dialog zeigt zusätzlich Nutzer mit Aktivität in den letzten 24h
const PRESENCE_POLL_MS = 10000; // Liste im geöffneten Dialog alle 10s neu laden

function usePresenceHeartbeat(username) {
  useEffect(() => {
    ...
    const ping = () => {
      if (cancelled) return;
      supabase.rpc('ping_presence', { p_username: username }).catch(() => {
        // Ein einzelner fehlgeschlagener Ping ist unkritisch – der nächste folgt automatisch.
      });
    };

    ping();
    const intervalId = setInterval(ping, PRESENCE_HEARTBEAT_MS);
...
/* ============================================================================
 * Komponente: "Wer ist online?"
 * ----------------------------------------------------------------------------
 * Zeigt zwei Gruppen: aktuell online (grün leuchtend) und zuletzt online
 * innerhalb der letzten 24h (hellgrau, mit "hh:min"-Angabe). Aktualisiert sich
 * selbstständig alle PRESENCE_POLL_MS, solange der Dialog geöffnet ist.
 * ==========================================================================*/
function formatSinceOffline(lastSeenIso, nowMs) {
  const diffMs = Math.max(0, nowMs - new Date(lastSeenIso).getTime());
...
function OnlineUsersModal({ onClose, currentUsername }) {
  ...
  const refresh = useCallback(async () => {
    ...
    const since = new Date(Date.now() - PRESENCE_RECENT_WINDOW_MS).toISOString();
    const { data, error: dbError } = await supabase
      .from('presence')
  ...
  useEffect(() => {
    refresh();
    const intervalId = setInterval(refresh, PRESENCE_POLL_MS);
  ...
  const { online, recentlyOffline } = useMemo(() => {
    const onlineList = [];
    const offlineList = [];
    for (const row of rows) {
      const lastSeenMs = new Date(row.last_seen).getTime();
      if (now - lastSeenMs <= PRESENCE_ONLINE_THRESHOLD_MS) onlineList.push(row);
      else offlineList.push(row);
    }
    onlineList.sort((a, b) => a.username.localeCompare(b.username));
    offlineList.sort((a, b) => new Date(b.last_seen) - new Date(a.last_seen));
    return { online: onlineList, recentlyOffline: offlineList };
  }, [rows, now]);

  const isEmpty = !error && !loading && online.length === 0 && recentlyOffline.length === 0;

  return html`
    <${Modal} title="Wer ist online?" onClose=${onClose}>
      <div class="flex flex-col gap-1.5">
        ${error && html`<p class="text-bad text-sm">${error}<//>`}
        ${!error && loading && html`<p class="text-muted text-sm">Lade…<//>`}
        ${isEmpty && html`<p class="text-muted text-sm">Gerade niemand online oder in den letzten 24h aktiv.<//>`}

        ${online.map((row) => html`
          <div key=${`online-${row.username}`} class="flex items-center justify-between px-3 py-2.5 rounded-xl bg-base">
            <div class="flex items-center gap-2.5">
              <span class="relative flex h-2 w-2 shrink-0">
                <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-good opacity-75"></span>
                <span class="relative inline-flex rounded-full h-2 w-2 bg-good"></span>
              </span>
              <span
                class="text-sm font-medium text-good"
                style=${{ textShadow: '0 0 8px rgba(61,220,132,0.65)' }}
              >${row.username}${row.username === currentUsername ? ' (du)' : ''}<//>
            </div>
            <span class="text-[11px] text-good/80 uppercase tracking-widest shrink-0">Online<//>
          </div>
        `)}

        ${recentlyOffline.map((row) => html`
          <div key=${`offline-${row.username}`} class="flex items-center justify-between px-3 py-2.5 rounded-xl bg-base">
            <span class="text-sm font-medium text-muted">${row.username}${row.username === currentUsername ? ' (du)' : ''}<//>
            <span class="text-[11px] text-muted tabular-digits shrink-0">zuletzt online ${formatSinceOffline(row.last_seen, now)}<//>
          </div>
        `)}
      </div>
    <//>
...
function Header({ personalBest, timeFormat, username, nameStyle, isAdmin, onOpenLeaderboard, onOpenSettings, onOpenLogin, onOpenLogout, onOpenTutorial, onOpenOnline }) {
  const style = getNameStyle(nameStyle);
  return html`
    <div class="relative flex items-start justify-between px-4 pt-4 sm:px-6 sm:pt-6 select-none">
      ...
      <button
        ...
        aria-label="Wer ist online öffnen"
        class="absolute left-1/2 -translate-x-1/2 top-4 sm:top-6 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-panel border border-panelBorder text-[11px] sm:text-xs text-muted hover:text-white active:scale-95 transition whitespace-nowrap"
      >
        <span class="relative flex h-2 w-2 shrink-0">
          <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-good opacity-75"></span>
          <span class="relative inline-flex rounded-full h-2 w-2 bg-good"></span>
...
function App() {
  ...
  const { entries, loading, error, submitScore, adminRemoveEntry, adminBanUsername, adminResetAll } = useLeaderboard();
  const { strikes, addStrike, isLocked } = useStrikes();
  usePresenceHeartbeat(!isAdmin ? auth?.username ?? null : null);
  ...
  return html`
    <div class="h-full min-h-screen flex flex-col bg-base">
      ...
      ${modal === 'online' && html`<${OnlineUsersModal} onClose=${closeModal} currentUsername=${auth?.username ?? null} />`}
